...
images: ['/static/images/Building-Production-Grade-CICD-Pipelines-for-Microservices-thumb.png']
date: "2026-02-20"
...
export const ToolTable = () => {
  const rows = [
    ...
    ["CI Platform",          "GitHub Actions · GitLab CI · Tekton",  "Tekton for K8s-native; GitHub Actions for ecosystem breadth"],
    ["Container Registry",   "AWS ECR · GCP Artifact Registry · Harbor", "Harbor for self-hosted with policy enforcement"],
    ["IaC",                  "Terraform + Helm",                     "Terraform for cloud resources; Helm for Kubernetes workloads"],
    ["GitOps Operator",      "ArgoCD · Flux",                        "ArgoCD has richer UI; Flux is more lightweight"],
    ["Progressive Delivery", "Argo Rollouts · Flagger",              "Automated canary analysis with metric-based promotion"],
    ["Secrets Management",   "HashiCorp Vault · AWS Secrets Manager","External Secrets Operator bridges Vault ↔ K8s"],
    ["Observability",        "Prometheus + Grafana + Loki + Tempo",  "Full open-source stack; Datadog for managed option"],
    ["Testing",              "Pact · k6 · Playwright · TestContainers","One tool per testing pyramid layer"],
    ["Security",             "Trivy · Semgrep · Snyk · Checkov",     "One tool per security domain; all integrated into CI"],
  ];
  return (
...
<div style={{ fontFamily: "'DM Mono', monospace", fontSize: "12px", color: "#94a3b8", display: "flex", gap: "20px", flexWrap: "wrap", margin: "16px 0 32px" }}>
  ...
  <span>🔖 DevOps · Kubernetes · GitOps</span>
</div>
...
<DiagramWrap label="Monorepo vs Polyrepo Comparison">
  <svg viewBox="0 0 820 300" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="205" y="195" textAnchor="middle" fill="#10b981" fontSize="11" fontFamily="monospace">✓ Atomic cross-service changes</text>
    <text x="205" y="215" textAnchor="middle" fill="#10b981" fontSize="11" fontFamily="monospace">✓ Unified tooling and standards</text>
    <text x="205" y="235" textAnchor="middle" fill="#10b981" fontSize="11" fontFamily="monospace">✓ Affected-only builds (Nx/Bazel)</text>
    <text x="205" y="258" textAnchor="middle" fill="#f59e0b" fontSize="11" fontFamily="monospace">△ More complex pipeline logic</text>
    ...
    <text x="615" y="195" textAnchor="middle" fill="#10b981" fontSize="11" fontFamily="monospace">✓ Full team autonomy</text>
    <text x="615" y="215" textAnchor="middle" fill="#10b981" fontSize="11" fontFamily="monospace">✓ Simple, isolated pipelines</text>
    <text x="615" y="235" textAnchor="middle" fill="#f59e0b" fontSize="11" fontFamily="monospace">△ Cross-service refactoring is hard</text>
    <text x="615" y="258" textAnchor="middle" fill="#f59e0b" fontSize="11" fontFamily="monospace">△ Tooling drift between teams</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="End-to-End Pipeline Flow">
  <svg viewBox="0 0 820 200" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="100" y="78" textAnchor="middle" fill="#a78bfa" fontSize="11" fontWeight="700" fontFamily="monospace">① CI</text>
    <text x="100" y="100" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Lint · Unit Tests</text>
    <text x="100" y="116" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Build Image · Scan</text>
    ...
    <text x="298" y="78" textAnchor="middle" fill="#67e8f9" fontSize="11" fontWeight="700" fontFamily="monospace">② Staging</text>
    <text x="298" y="100" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Integration Tests</text>
    <text x="298" y="116" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Contract · E2E · Perf</text>
    ...
    <text x="496" y="78" textAnchor="middle" fill="#fcd34d" fontSize="11" fontWeight="700" fontFamily="monospace">③ Pre-Prod Gate</text>
    <text x="496" y="100" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Approval / Policy</text>
    <text x="496" y="116" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Security · Compliance</text>
    ...
    <text x="707" y="78" textAnchor="middle" fill="#6ee7b7" fontSize="11" fontWeight="700" fontFamily="monospace">④ Production</text>
    <text x="707" y="100" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Canary / Blue-Green</text>
    <text x="707" y="116" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Smoke Tests · Monitor</text>
    ...
    <text x="400" y="25" textAnchor="middle" fill="#6ee7b7" fontSize="10" fontWeight="600" fontFamily="monospace">🔒 Immutable Artifact — same SHA promoted through all stages</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="Multi-Stage Docker Build Flow">
  <svg viewBox="0 0 860 220" width="100%" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <marker id="arr2" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
        <path d="M0,0 L0,6 L8,3 z" fill="#10b981" opacity="0.9"/>
      </marker>
    </defs>

    {/* ── BUILD STAGE ── */}
    <rect x="20" y="30" width="230" height="150" rx="8" fill="#111827" stroke="#7c3aed" strokeWidth="1.5"/>
    <rect x="20" y="30" width="230" height="30" rx="8" fill="#7c3aed" opacity="0.25"/>
    <text x="135" y="51" textAnchor="middle" fill="#a78bfa" fontSize="12" fontWeight="700" fontFamily="monospace">BUILD STAGE</text>
    <text x="135" y="82"  textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">FROM golang:1.22 AS builder</text>
    <text x="135" y="100" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">COPY . .</text>
    <text x="135" y="118" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">RUN go test ./...</text>
    <text x="135" y="136" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">RUN go build -o /app</text>
    <text x="135" y="168" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">~900MB image</text>

    {/* ── ARROW 1: BUILD → FINAL (label above) ── */}
    <text x="310" y="88" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">COPY --from=builder</text>
    <text x="310" y="101" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">/app binary only</text>
    <line x1="252" y1="115" x2="368" y2="115" stroke="#10b981" strokeWidth="1.5" markerEnd="url(#arr2)"/>

    {/* ── FINAL STAGE ── */}
    <rect x="370" y="30" width="220" height="150" rx="8" fill="#111827" stroke="#10b981" strokeWidth="1.5"/>
    <rect x="370" y="30" width="220" height="30" rx="8" fill="#10b981" opacity="0.15"/>
    <text x="480" y="51"  textAnchor="middle" fill="#6ee7b7" fontSize="12" fontWeight="700" fontFamily="monospace">FINAL STAGE</text>
    <text x="480" y="82"  textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">FROM gcr.io/distroless/static</text>
    <text x="480" y="100" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">COPY /app /app</text>
    <text x="480" y="118" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">USER nonroot</text>
    <text x="480" y="136" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">ENTRYPOINT ["/app"]</text>
    <text x="480" y="168" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">~12MB image ✓</text>

    {/* ── ARROW 2: FINAL → REGISTRY ── */}
    <line x1="592" y1="115" x2="618" y2="115" stroke="#10b981" strokeWidth="1.5" markerEnd="url(#arr2)"/>

    {/* ── REGISTRY ── */}
    <rect x="620" y="50" width="220" height="120" rx="8" fill="#111827" stroke="#00e5ff" strokeWidth="1.5"/>
    <rect x="620" y="50" width="220" height="30" rx="8" fill="#00e5ff" opacity="0.1"/>
    <text x="730" y="71"  textAnchor="middle" fill="#67e8f9" fontSize="12" fontWeight="700" fontFamily="monospace">REGISTRY</text>
    <text x="730" y="100" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">tag: sha-a3f7c2b1</text>
    <text x="730" y="118" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">✓ Signed  ✓ CVE-scanned</text>
    <text x="730" y="136" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">✓ Retention policy applied</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="GitOps Deployment Model (ArgoCD / Flux)">
  <svg viewBox="0 0 860 240" width="100%" xmlns="http://www.w3.org/2000/svg">
    <defs>
      ...
      <marker id="arr5" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
        <path d="M0,0 L0,6 L8,3 z" fill="#f59e0b" opacity="0.9"/>
      </marker>
    </defs>

    {/* ── DEVELOPER (cy=100) ── */}
    <rect x="10" y="70" width="110" height="70" rx="6" fill="#1a2235" stroke="#7c3aed" strokeWidth="1.5"/>
    <text x="65" y="102" textAnchor="middle" fontSize="22">👩‍💻</text>
    <text x="65" y="128" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">Developer</text>

    {/* ── ARROW 1: Dev → CI (label above) ── */}
    <text x="162" y="93" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">git push</text>
    <line x1="122" y1="105" x2="198" y2="105" stroke="#00e5ff" strokeWidth="1.5" markerEnd="url(#arr3)"/>

    {/* ── CI SYSTEM ── */}
    <rect x="200" y="70" width="140" height="70" rx="6" fill="#1a2235" stroke="#00e5ff" strokeWidth="1.5"/>
    <rect x="200" y="70" width="140" height="26" rx="6" fill="#00e5ff" opacity="0.1"/>
    <text x="270" y="88"  textAnchor="middle" fill="#67e8f9" fontSize="11" fontWeight="700" fontFamily="monospace">CI System</text>
    <text x="270" y="112" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Build · Test · Push</text>
    <text x="270" y="128" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Image to Registry</text>

    {/* ── ARROW 2: CI → GitOps Repo (label above) ── */}
    <text x="362" y="93" textAnchor="middle" fill="#fcd34d" fontSize="9" fontFamily="monospace">update image tag</text>
    <line x1="342" y1="105" x2="398" y2="105" stroke="#f59e0b" strokeWidth="1.5" markerEnd="url(#arr5)"/>

    {/* ── GITOPS REPO ── */}
    <rect x="400" y="60" width="150" height="100" rx="6" fill="#1a2235" stroke="#f59e0b" strokeWidth="1.5"/>
    <rect x="400" y="60" width="150" height="26" rx="6" fill="#f59e0b" opacity="0.15"/>
    <text x="475" y="79"  textAnchor="middle" fill="#fcd34d" fontSize="11" fontWeight="700" fontFamily="monospace">GitOps Repo</text>
    <text x="475" y="104" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">manifests/</text>
    <text x="475" y="120" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">prod/values.yaml</text>
    <text x="475" y="138" textAnchor="middle" fill="#fcd34d" fontSize="9" fontFamily="monospace">image: sha-a3f7c2</text>

    {/* ── ARROW 3: GitOps Repo → K8s (label above, dashed pull) ── */}
    <text x="582" y="93" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">pull / sync (operator)</text>
    <line x1="552" y1="105" x2="608" y2="105" stroke="#10b981" strokeWidth="1.5" strokeDasharray="5,3" markerEnd="url(#arr4)"/>

    {/* ── KUBERNETES CLUSTER ── */}
    <rect x="610" y="40" width="230" height="160" rx="8" fill="#111827" stroke="#10b981" strokeWidth="1.5"/>
    <rect x="610" y="40" width="230" height="28" rx="8" fill="#10b981" opacity="0.1"/>
    <text x="725" y="60" textAnchor="middle" fill="#6ee7b7" fontSize="12" fontWeight="700" fontFamily="monospace">Kubernetes Cluster</text>
    <rect x="628" y="78" width="194" height="38" rx="4" fill="#0f2a1e" stroke="#10b981" strokeWidth="1"/>
    <text x="725" y="97"  textAnchor="middle" fill="#6ee7b7" fontSize="10" fontWeight="600" fontFamily="monospace">ArgoCD / Flux Operator</text>
    <text x="725" y="110" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">continuous sync loop</text>
    <rect x="628" y="128" width="88" height="30" rx="4" fill="#1a2235" stroke="#00e5ff" strokeWidth="1"/>
    <text x="672" y="147" textAnchor="middle" fill="#a5f3fc" fontSize="9" fontFamily="monospace">pod v2 ✓</text>
    <rect x="734" y="128" width="88" height="30" rx="4" fill="#1a2235" stroke="#00e5ff" strokeWidth="1"/>
    <text x="778" y="147" textAnchor="middle" fill="#a5f3fc" fontSize="9" fontFamily="monospace">pod v2 ✓</text>

    {/* ── ROLLBACK NOTE ── */}
    <text x="430" y="195" textAnchor="middle" fill="#f59e0b" fontSize="10" fontFamily="monospace">Rollback = git revert → instant reconcile</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="Canary Deployment — Automated Progressive Traffic Shifting">
  <svg viewBox="0 0 900 250" width="100%" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <marker id="arr6a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
        <path d="M0,0 L0,6 L8,3 z" fill="#94a3b8"/>
      </marker>
      <marker id="arr6b" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
        <path d="M0,0 L0,6 L8,3 z" fill="#00e5ff"/>
      </marker>
      <marker id="arr6c" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto">
        <path d="M0,0 L0,6 L8,3 z" fill="#f43f5e"/>
      </marker>
    </defs>

    {/* ── COLUMN HEADER LABELS ── */}
    <text x="215" y="22" textAnchor="middle" fill="#94a3b8" fontSize="10" fontWeight="600" fontFamily="monospace">Step 1</text>
    <text x="390" y="22" textAnchor="middle" fill="#94a3b8" fontSize="10" fontWeight="600" fontFamily="monospace">Step 2</text>
    <text x="570" y="22" textAnchor="middle" fill="#94a3b8" fontSize="10" fontWeight="600" fontFamily="monospace">Step 3 (auto-promote)</text>
    <text x="770" y="22" textAnchor="middle" fill="#f59e0b" fontSize="10" fontWeight="600" fontFamily="monospace">Rollback trigger</text>

    {/* ── LOAD BALANCER ── */}
    <rect x="10" y="90" width="120" height="55" rx="6" fill="#1a2235" stroke="#00e5ff" strokeWidth="1.5"/>
    <text x="70" y="114" textAnchor="middle" fill="#67e8f9" fontSize="11" fontWeight="700" fontFamily="monospace">Load Balancer</text>
    <text x="70" y="133" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Traffic routing</text>

    {/* ── LB → STABLE V1 (top fork) ── */}
    <polyline points="130,105 148,105 148,78 155,78" fill="none" stroke="#94a3b8" strokeWidth="1.2" strokeDasharray="3,2" markerEnd="url(#arr6a)"/>
    {/* ── LB → CANARY V2 (bottom fork) ── */}
    <polyline points="130,128 148,128 148,162 155,162" fill="none" stroke="#94a3b8" strokeWidth="1.2" strokeDasharray="3,2" markerEnd="url(#arr6a)"/>

    {/* ── STEP 1: STABLE V1 ── */}
    <rect x="157" y="55" width="120" height="46" rx="5" fill="#1a2235" stroke="#10b981" strokeWidth="1.5"/>
    <text x="217" y="76" textAnchor="middle" fill="#6ee7b7" fontSize="10" fontFamily="monospace">stable v1</text>
    <text x="217" y="92" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">95% traffic</text>

    {/* ── STEP 1: CANARY V2 ── */}
    <rect x="157" y="138" width="120" height="46" rx="5" fill="#0f2a1e" stroke="#f59e0b" strokeWidth="1.5"/>
    <text x="217" y="159" textAnchor="middle" fill="#fcd34d" fontSize="10" fontFamily="monospace">canary v2</text>
    <text x="217" y="175" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">5% traffic</text>

    {/* ── STEP 1 → STEP 2 ARROWS ── */}
    <line x1="279" y1="78"  x2="327" y2="78"  stroke="#00e5ff" strokeWidth="1.2" strokeDasharray="4,2" markerEnd="url(#arr6b)"/>
    <line x1="279" y1="161" x2="327" y2="161" stroke="#00e5ff" strokeWidth="1.2" strokeDasharray="4,2" markerEnd="url(#arr6b)"/>

    {/* ── STEP 2: STABLE V1 ── */}
    <rect x="329" y="55" width="120" height="46" rx="5" fill="#1a2235" stroke="#10b981" strokeWidth="1.5"/>
    <text x="389" y="76" textAnchor="middle" fill="#6ee7b7" fontSize="10" fontFamily="monospace">stable v1</text>
    <text x="389" y="92" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">50% traffic</text>

    {/* ── STEP 2: CANARY V2 ── */}
    <rect x="329" y="138" width="120" height="46" rx="5" fill="#0f2a1e" stroke="#f59e0b" strokeWidth="1.5"/>
    <text x="389" y="159" textAnchor="middle" fill="#fcd34d" fontSize="10" fontFamily="monospace">canary v2</text>
    <text x="389" y="175" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">50% traffic</text>
    <text x="389" y="210" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">✓ metrics healthy</text>

    {/* ── STEP 2 → STEP 3 ARROW (single, merging) ── */}
    <line x1="451" y1="78"  x2="500" y2="78"  stroke="#00e5ff" strokeWidth="1.2" strokeDasharray="4,2"/>
    <line x1="451" y1="161" x2="500" y2="161" stroke="#00e5ff" strokeWidth="1.2" strokeDasharray="4,2"/>
    <line x1="500" y1="78" x2="500" y2="161" stroke="#00e5ff" strokeWidth="1.2" strokeDasharray="4,2"/>
    <line x1="500" y1="118" x2="508" y2="118" stroke="#00e5ff" strokeWidth="1.2" markerEnd="url(#arr6b)"/>

    {/* ── STEP 3: V2 100% ── */}
    <rect x="510" y="88" width="140" height="60" rx="5" fill="#0f2a1e" stroke="#10b981" strokeWidth="2"/>
    <text x="580" y="113" textAnchor="middle" fill="#6ee7b7" fontSize="11" fontWeight="700" fontFamily="monospace">v2 — 100%</text>
    <text x="580" y="131" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">v1 pods terminated</text>

    {/* ── STEP 3 → ROLLBACK ARROW ── */}
    <line x1="652" y1="118" x2="682" y2="118" stroke="#f43f5e" strokeWidth="1.2" strokeDasharray="4,2" markerEnd="url(#arr6c)"/>

    {/* ── ROLLBACK BOX ── */}
    <rect x="684" y="82" width="200" height="72" rx="5" fill="#2d1515" stroke="#f43f5e" strokeWidth="1.5"/>
    <text x="784" y="108" textAnchor="middle" fill="#fb7185" fontSize="11" fontWeight="700" fontFamily="monospace">⚡ Auto-Rollback</text>
    <text x="784" y="126" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">error rate {">"} threshold</text>
    <text x="784" y="143" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">v1 restored instantly</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="Microservices Testing Pyramid">
  <svg viewBox="0 0 820 310" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="410" y="264" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">Hundreds · Seconds · In-process · High coverage of business logic</text>
    ...
    <text x="410" y="197" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">TestContainers · Real DB/Queue · Minutes</text>
    ...
    <text x="410" y="138" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">Pact · API compatibility · Per-service CI</text>
    ...
    <text x="410" y="89" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">Few · Slow · Staging only</text>
    <text x="690" y="248" fill="#a78bfa" fontSize="10" fontFamily="monospace">→ Fast · Many · Cheap</text>
    <text x="690" y="185" fill="#67e8f9" fontSize="10" fontFamily="monospace">→ Medium speed</text>
    <text x="690" y="130" fill="#6ee7b7" fontSize="10" fontFamily="monospace">→ Critical layer!</text>
    <text x="690" y="78" fill="#fcd34d" fontSize="10" fontFamily="monospace">→ Slow · Few · Staging</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="Security Checkpoints Across the Pipeline">
  <svg viewBox="0 0 820 180" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="30" y="175" fill="#94a3b8" fontSize="9" fontFamily="monospace">← earlier in pipeline (cheaper to fix)</text>
    <text x="680" y="175" fill="#94a3b8" fontSize="9" fontFamily="monospace">later →</text>
  </svg>
</DiagramWrap>
...
<DiagramWrap label="Observability Stack — Three Pillars + Deployment Markers">
  <svg viewBox="0 0 820 220" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="130" y="63" textAnchor="middle" fill="#a78bfa" fontSize="13" fontWeight="700" fontFamily="monospace">📋 LOGS</text>
    ...
    <text x="410" y="63" textAnchor="middle" fill="#67e8f9" fontSize="13" fontWeight="700" fontFamily="monospace">📊 METRICS</text>
    <text x="410" y="96" textAnchor="middle" fill="#94a3b8" fontSize="10" fontFamily="monospace">4 Golden Signals:</text>
    <text x="410" y="114" textAnchor="middle" fill="#67e8f9" fontSize="10" fontFamily="monospace">Latency · Traffic · Errors · Saturation</text>
    ...
    <text x="690" y="63" textAnchor="middle" fill="#6ee7b7" fontSize="13" fontWeight="700" fontFamily="monospace">🔍 TRACES</text>
...
<DiagramWrap label="Incident Response Decision Tree">
  <svg viewBox="0 0 820 280" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="410" y="113" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">error rate · latency · readiness</text>
    <line x1="310" y1="100" x2="240" y2="100" stroke="#10b981" strokeWidth="1.5" markerEnd="url(#arr7)"/>
    <text x="272" y="92" textAnchor="middle" fill="#6ee7b7" fontSize="9" fontFamily="monospace">✓ healthy</text>
    ...
    <text x="540" y="92" textAnchor="middle" fill="#fb7185" fontSize="9" fontFamily="monospace">✗ degraded</text>
    ...
    <text x="652" y="113" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">{"<"}60 seconds · automated</text>
    ...
    <text x="652" y="188" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">on-call paged · runbook linked</text>
    ...
    <text x="652" y="261" textAnchor="middle" fill="#94a3b8" fontSize="9" fontFamily="monospace">git revert → operator reconciles</text>
    <rect x="60" y="155" width="240" height="65" rx="6" fill="#1a2235" stroke="#f59e0b" strokeWidth="1"/>
    <text x="180" y="175" textAnchor="middle" fill="#fcd34d" fontSize="10" fontWeight="600" fontFamily="monospace">⚠️ Database Migration Rule</text>
