...
subtitle: "A complete end-to-end walkthrough — Python FastAPI microservices, AKS, ACR, GitHub Actions"
date: "2026-02-20"
author: "Ali Haidry"
tags: ["Azure", "AKS", "GitHub Actions", "FastAPI", "Python", "CI/CD", "Microservices", "E-Commerce"]
readTime: "35 min read"
project: "ShopWave E-Commerce Platform"
stack: "Python · FastAPI · Azure AKS · ACR · Azure DB · GitHub Actions"
---
...
<div style={{ padding: "80px 0 40px", borderBottom: `1px solid ${T.border}` }}>
  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "24px" }}>
    ...
    <Badge color={T.accent4}>Production Grade</Badge>
  ...
  <p style={{ fontSize: "1.25rem", color: T.muted, maxWidth: "680px", margin: "0 0 16px" }}>
    A complete, production-grade E-Commerce platform built with Python FastAPI microservices, deployed on Azure Kubernetes Service via GitHub Actions — from zero to live, nothing skipped.
  </p>
...
<div style={{ padding: "40px 0 20px" }}>
  <p>This is not a tutorial with placeholder code. Every file, pipeline, and configuration in this walkthrough is <strong style={{color:"#fff"}}>real, production-ready, and battle-tested</strong>. By the end you will have a complete monorepo containing four Python FastAPI microservices, a full Azure infrastructure stack, and GitHub Actions pipelines that take code from a developer's laptop to a live Kubernetes cluster — with security scanning, automated testing, canary deployments, and instant rollback capability built in.</p>
  <p>We are building <strong style={{color:"#fff"}}>ShopWave</strong> — a modern e-commerce backend with an API Gateway, Auth Service, Product Service, and Order Service. Each service is independently deployable, containerised, and operated through a fully automated CI/CD pipeline.</p>
</div>
...
<DiagramBox label="ShopWave — Full System Architecture">
  <svg viewBox="0 0 860 420" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="782" y="288" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">CI · CD · Security</text>
    <text x="782" y="304" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">5 pipelines</text>
...
<p>ShopWave lives in a single monorepo. GitHub Actions uses path-based filtering to detect which services changed and runs only the relevant pipelines — so deploying a hotfix to <code style={{color:T.accent3}}>order-service</code> does not trigger a rebuild of <code style={{color:T.accent3}}>auth-service</code>.</p>
...
├── infrastructure/
│   └── terraform/
│       ├── main.tf                    ← AKS + ACR + Azure DB provisioning
│       ├── variables.tf
│       ├── outputs.tf
│       └── modules/
│           ├── aks/
...
│           └── externalsecret.yaml    ← Azure Key Vault secrets
│
├── k8s/
│   ├── namespaces.yaml
...
│
├── scripts/
│   ├── bootstrap-azure.sh             ← One-shot Azure resource setup
│   └── seed-db.sh
│
...
{/* ══════════════════════════════════════════════════
    SECTION 7 — AZURE SECRETS & RBAC
══════════════════════════════════════════════════ */}
<SectionHeader num="07" title="SECRETS, RBAC & AZURE KEY VAULT" sub="Zero secrets in code, environment variables, or pipeline logs." />

<p>All secrets are stored in Azure Key Vault and injected into pods at runtime via the External Secrets Operator. The pipeline itself uses Workload Identity (OIDC) — there are no stored Azure credentials in GitHub Secrets beyond the federated identity configuration.</p>
...
<CodeBlock lang="yaml" filename="k8s/rbac.yaml">{`# Service accounts with least-privilege bindings per namespace
---
apiVersion: v1
kind: ServiceAccount
metadata:
  name: shopwave-workload
  namespace: shopwave-production
  annotations:
    # Workload Identity — links K8s SA to Azure Managed Identity
    azure.workload.identity/client-id: \$AZURE_MANAGED_IDENTITY_CLIENT_ID
---
apiVersion: rbac.authorization.k8s.io/v1
kind: Role
metadata:
  name: shopwave-service-role
  namespace: shopwave-production
rules:
  - apiGroups: [""]
  ...
  - apiGroups: [""]
    resources: [secrets]
    verbs:     []        # No direct secret access — ESO handles this
---
apiVersion: rbac.authorization.k8s.io/v1
kind: RoleBinding
metadata:
  name: shopwave-service-binding
  namespace: shopwave-production
subjects:
  - kind:      ServiceAccount
    name:      shopwave-workload
...
<CodeBlock lang="bash" filename="scripts/bootstrap-azure.sh">{`#!/usr/bin/env bash
# One-shot Azure resource bootstrap for ShopWave
...
RG="shopwave-rg"
LOCATION="eastus"
AKS_NAME="shopwave-aks"
ACR_NAME="shopwaveacr"          # Must be globally unique
KV_NAME="shopwave-kv"
PG_SERVER="shopwave-pg"
PG_ADMIN="shopwaveadmin"
SUBSCRIPTION_ID=\$(az account show --query id -o tsv)

echo "🔧 Bootstrapping ShopWave Azure resources..."
...
az aks create \
  ...
  --generate-ssh-keys \
  --output none
echo "✅ AKS cluster: \$AKS_NAME (3 nodes)"
...
for DB in auth_db products_db orders_db; do
...
echo "✅ PostgreSQL: \$PG_SERVER (auth_db, products_db, orders_db)"

# ── 6. Store secrets in Key Vault ──────────────────────────
PG_HOST="\${PG_SERVER}.postgres.database.azure.com"
az keyvault secret set --vault-name "\$KV_NAME" --name "shopwave-prod-db-url"         --value "postgresql+asyncpg://\${PG_ADMIN}:\${PG_PASSWORD}@\${PG_HOST}/auth_db?ssl=require"    --output none
az keyvault secret set --vault-name "\$KV_NAME" --name "shopwave-prod-jwt-secret"      --value "\$(openssl rand -hex 64)"  --output none
az keyvault secret set --vault-name "\$KV_NAME" --name "shopwave-prod-allowed-origins"  --value "https://shopwave.io"       --output none
echo "✅ Secrets stored in Key Vault"

# ── 7. Configure GitHub Actions OIDC ───────────────────────
GH_ORG="your-github-org"
GH_REPO="shopwave"
APP_NAME="shopwave-gh-actions"
...
az role assignment create --assignee "\$APP_ID" --role "AcrPush"              --scope \$(az acr show -n "\$ACR_NAME" -g "\$RG" --query id -o tsv) --output none
az role assignment create --assignee "\$APP_ID" --role "Azure Kubernetes Service Cluster User Role" --scope \$(az aks show -n "\$AKS_NAME" -g "\$RG" --query id -o tsv) --output none
az role assignment create --assignee "\$APP_ID" --role "Key Vault Secrets User" --scope \$(az keyvault show -n "\$KV_NAME" -g "\$RG" --query id -o tsv) --output none
...
<DiagramBox label="Required GitHub Secrets">
  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem", fontFamily: "monospace" }}>
    ...
    <tbody>
      {[
        ["AZURE_CLIENT_ID",       "bootstrap-azure.sh output",         "All CD pipelines",          "OIDC federated identity — no password"],
        ["AZURE_TENANT_ID",       "az account show --query tenantId",  "All CD pipelines",          "Your Azure AD tenant"],
        ["AZURE_SUBSCRIPTION_ID", "az account show --query id",        "All CD pipelines",          "Target subscription"],
        ["ACR_LOGIN_SERVER",      "shopwaveacr.azurecr.io",            "CI build + CD deploy",      "Full ACR hostname"],
        ["ACR_USERNAME",          "az acr credential show",            "CI build (docker login)",   "ACR admin username"],
        ["ACR_PASSWORD",          "az acr credential show",            "CI build (docker login)",   "Rotate regularly"],
        ["SLACK_WEBHOOK_URL",     "Slack app settings",                "All pipelines (notify)",    "Incoming webhook URL"],
...
<DiagramBox label="Complete Developer → Production Journey">
  <svg viewBox="0 0 860 520" width="100%" xmlns="http://www.w3.org/2000/svg">
    <defs>
    ...
    {/* Timeline line */}
    <line x1="60" y1="30" x2="60" y2="490" stroke="#1e2840" strokeWidth="2"/>
...
<DiagramBox label="Security Controls — Layer by Layer">
  <svg viewBox="0 0 860 340" width="100%" xmlns="http://www.w3.org/2000/svg">
    {[
      ...
      { y: 200, label: "RUNTIME",     color: "#22c55e", items: ["runAsNonRoot: true · readOnlyRootFilesystem: true", "capabilities: drop [ALL] — zero Linux capabilities", "Azure Network Policy — default deny, explicit allow-list"] },
      { y: 290, label: "CREDENTIALS", color: "#eab308", items: ["OIDC Workload Identity — zero static credentials", "Azure Key Vault — secrets never in env vars or logs", "Secret rotation via Key Vault versioning + ESO refresh"] },
...
<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "14px", margin: "28px 0" }}>
  {[
    ...
    { icon: "⚙️",  title: "6 GitHub Actions Pipelines",      desc: "4 per-service CI pipelines + staging CD + production CD — path-filtered, OIDC-authenticated, Slack-notified" },
    { icon: "🔒", title: "End-to-End Security",              desc: "Semgrep SAST · Trivy image scan · Cosign signing · OIDC no-credential auth · Key Vault secrets · zero-trust network policies" },
    ...
    { icon: "☁️",  title: "Full Azure Stack",                desc: "AKS + ACR + PostgreSQL Flexible + Key Vault + Azure Monitor · bootstrapped with a single idempotent shell script" },
  ].map(({ icon, title, desc }) => (
...
<Callout type="success" title="Next steps">
  From here, natural extensions include: adding <strong>Argo Rollouts</strong> for canary deployments with automated metric-based promotion, integrating <strong>Azure Application Insights</strong> for distributed tracing across all four services, adding a <strong>contract testing</strong> job to each CI pipeline using Pact, and wiring <strong>Azure Cost Management</strong> alerts into the pipeline notifications.
</Callout>

<div style={{ borderTop: `1px solid ${T.border}`, marginTop: "60px", paddingTop: "32px", textAlign: "center" }}>
  <p style={{ fontFamily: "monospace", fontSize: "12px", color: T.muted }}>ShopWave · Python FastAPI · Azure AKS · GitHub Actions · Production-Grade CI/CD · February 2026</p>
</div>
