---
title: "ShopWave: Production-Grade E-Commerce CI/CD on Azure (In-Progress)"
subtitle: "A complete end-to-end walkthrough — Python FastAPI microservices, AKS, ACR, GitHub Actions, Terraform"
date: "2026-02-20"
...
stack: "Python · FastAPI · Azure AKS · ACR · Azure DB · Terraform · GitHub Actions"
---
...
export const Callout = ({ type = "info", title, children }) => {
  const map = {
    info:    { border: T.accent,  bg: "#0a1628", icon: "ℹ" },
    warn:    { border: T.warn,    bg: "#1a1400", icon: "⚠" },
    success: { border: T.accent3, bg: "#0a1a0e", icon: "✓" },
    danger:  { border: T.danger,  bg: "#1a0a0a", icon: "✗" },
    tip:     { border: T.accent4, bg: "#120a1a", icon: "💡" },
  };
...
<div style={{ padding: "80px 0 40px", borderBottom: `1px solid ${T.border}` }}>
  <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "24px" }}>
    ...
    <Badge color={T.accent3}>FastAPI · Python</Badge>
  ...
  <p style={{ fontSize: "1.25rem", color: T.muted, maxWidth: "680px", margin: "0 0 16px" }}>
    A complete, production-grade E-Commerce platform built with Python FastAPI microservices, deployed on Azure Kubernetes Service via GitHub Actions — infrastructure provisioned entirely with Terraform. From zero to live, nothing skipped.
  </p>
  <div style={{ fontFamily: "monospace", fontSize: "12px", color: T.muted, display: "flex", gap: "24px", flexWrap: "wrap" }}>
    ...
    <span>☁️ Azure AKS + ACR + PostgreSQL</span>
    <span>🏗️ Terraform 1.7+</span>
...
<div style={{ padding: "40px 0 20px" }}>
  <p>This is not a tutorial with placeholder code. Every file, pipeline, and configuration in this walkthrough is <strong style={{color:"#fff"}}>real, production-ready, and battle-tested</strong>. By the end you will have a complete monorepo containing four Python FastAPI microservices, a full Azure infrastructure stack provisioned with Terraform, and GitHub Actions pipelines that take code from a developer's laptop to a live Kubernetes cluster — with security scanning, automated testing, canary deployments, and instant rollback capability built in.</p>
  <p>We are building <strong style={{color:"#fff"}}>ShopWave</strong> — a modern e-commerce backend with an API Gateway, Auth Service, Product Service, and Order Service. Each service is independently deployable, containerised, and operated through a fully automated CI/CD pipeline. The entire Azure infrastructure — AKS cluster, Container Registry, PostgreSQL databases, Key Vault, private networking, and GitHub OIDC federation — is declared as Terraform code and applied through its own dedicated pipeline.</p>
</div>

{/* ── ARCHITECTURE DIAGRAM ── */}
<DiagramBox label="ShopWave — Full System Architecture">
  <svg viewBox="0 0 860 420" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="230" y="212" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">Rate limit · Auth · Route</text>
    <text x="230" y="235" textAnchor="middle" fill="#22c55e" fontSize="9" fontFamily="monospace">nginx ingress → FastAPI</text>
    ...
    <text x="435" y="140" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">:8001  JWT · OAuth2</text>
    <text x="435" y="156" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">Azure AD · bcrypt</text>
    ...
    <text x="435" y="205" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">:8002  Catalog · Search</text>
    <text x="435" y="221" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">Inventory · Pricing</text>
    ...
    <text x="435" y="272" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">:8003  Cart · Checkout</text>
    <text x="435" y="288" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">Payment · Fulfilment</text>
    ...
    <text x="633" y="248" textAnchor="middle" fill="#22c55e" fontSize="9" fontFamily="monospace">SSL · Private endpoint</text>
    ...
    <text x="782" y="288" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">CI · CD · Terraform</text>
    ...
    <text x="633" y="328" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">Prometheus · Grafana</text>
    <text x="633" y="344" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">App Insights · Alerts</text>
...
══════════════════════════════════════════════════ */}
<SectionHeader num="01" title="PROJECT STRUCTURE & MONOREPO LAYOUT" sub="Every file, every directory — nothing hand-wavy." />

<p>ShopWave lives in a single monorepo. GitHub Actions uses path-based filtering to detect which services changed and runs only the relevant pipelines — so deploying a hotfix to <code style={{color:T.accent3}}>order-service</code> does not trigger a rebuild of <code style={{color:T.accent3}}>auth-service</code>. A separate <code style={{color:T.accent3}}>terraform.yml</code> pipeline manages all Azure infrastructure changes, keeping application and infrastructure deployments completely independent.</p>

<FileTree>{`shopwave/
├── .github/
│   └── workflows/
│       ├── ci-auth-service.yml        ← CI for auth-service (PR + merge)
│       ├── ci-product-service.yml     ← CI for product-service
│       ├── ci-order-service.yml       ← CI for order-service
│       ├── ci-api-gateway.yml         ← CI for api-gateway
│       ├── cd-staging.yml             ← Deploy ALL changed services → staging
│       ├── cd-production.yml          ← Promote staging → production (manual gate)
│       └── terraform.yml              ← Infrastructure CI/CD (plan on PR · apply on dispatch)
│
├── services/
│   ├── api-gateway/
│   │   ├── app/
...
├── infrastructure/
│   └── terraform/
│       ├── main.tf                    ← Root: wires all modules + OIDC federation
│       ├── variables.tf               ← All input variables with validation
│       ├── outputs.tf                 ← GitHub Secrets summary + resource IDs
│       ├── terraform.tfvars.example   ← Copy → terraform.tfvars to configure
│       ├── modules/
...
│       │   └── keyvault/              ← Azure Key Vault · seeds JWT + origins
│       ├── environments/
│       │   ├── staging/staging.tfvars
│       │   └── production/production.tfvars
│       └── scripts/
│           └── bootstrap-tfstate.sh   ← One-time: creates remote state backend
│
├── helm/
│   └── shopwave/
│       ├── Chart.yaml
...
│           └── externalsecret.yaml    ← Azure Key Vault secret injection
│
├── k8s/
...
│
├── scripts/
│   └── init-databases.sql             ← Local dev: create all three databases
│
...
{/* ══════════════════════════════════════════════════
    SECTION 2 — FASTAPI MICROSERVICES
══════════════════════════════════════════════════ */}
<SectionHeader num="02" title="THE PYTHON FASTAPI MICROSERVICES" sub="Real application code — auth, products, orders, and gateway." />
...
<CodeBlock lang="dockerfile" filename="services/auth-service/Dockerfile">{`# ── Stage 1: build & test ──────────────────────────────
...
{/* ══════════════════════════════════════════════════
    SECTION 3 — GITHUB ACTIONS CI PIPELINES
══════════════════════════════════════════════════ */}
<SectionHeader num="03" title="GITHUB ACTIONS — CI PIPELINES" sub="Per-service pipelines with lint, test, security scan, and image push to ACR." />
...
<DiagramBox label="CI Pipeline Flow — Per Service">
  <svg viewBox="0 0 860 130" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    {[
      { x: 10,  label: "① CHECKOUT", sub: "actions/checkout@v4", color: "#a855f7" },
      { x: 180, label: "② LINT + FORMAT", sub: "ruff · black · mypy", color: "#3b82f6" },
      { x: 350, label: "③ UNIT TESTS", sub: "pytest --cov ≥80%", color: "#22c55e" },
      { x: 520, label: "④ SECURITY SCAN", sub: "Trivy + Semgrep", color: "#eab308" },
      { x: 690, label: "⑤ BUILD + PUSH", sub: "ACR · sha tag", color: "#f97316" },
    ...
    <text x="430" y="120" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">Fails at any stage → PR blocked · Slack alert · No image pushed</text>
  </svg>
</DiagramBox>

<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 12px"}}>CI Pipeline — auth-service</h3>

<CodeBlock lang="yaml" filename=".github/workflows/ci-auth-service.yml">{`name: CI — auth-service
...
<Callout type="tip" title="Path-based CI — other services">
  The CI pipelines for <code>product-service</code>, <code>order-service</code>, and <code>api-gateway</code> are identical in structure — only the <code>paths</code> filter, <code>SERVICE</code> env var, <code>IMAGE_NAME</code>, exposed port, and any service-specific test environment variables change. This pattern means a single-service hotfix triggers exactly one CI pipeline run, not four.
</Callout>

{/* ══════════════════════════════════════════════════
    SECTION 4 — CD STAGING PIPELINE
══════════════════════════════════════════════════ */}
<SectionHeader num="04" title="GITHUB ACTIONS — CD STAGING PIPELINE" sub="Automatic deploy to AKS staging on every merge to main." />
...
<DiagramBox label="CD Staging Flow — Triggered after CI success on main">
  <svg viewBox="0 0 860 150" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    {[
      ...
      { x: 590, label: "PASS ✓", sub: "Notify + tag ready", color: "#22c55e" },
    ].map(({ x, label, sub, color }, i) => (
      <g key={i}>
...
<CodeBlock lang="yaml" filename=".github/workflows/cd-staging.yml">{`name: CD — Deploy to Staging
...
jobs:
  deploy-staging:
    name: "Deploy \${{ github.event.workflow_run.name }} → Staging"
    runs-on: ubuntu-latest
    ...
    steps:
      ...
      - name: Resolve service and image tag
        ...
        run: |
          ...
          case "\$WORKFLOW" in
            "CI — auth-service")    SVC="auth-service";    PORT=8001 ;;
            "CI — product-service") SVC="product-service"; PORT=8002 ;;
            "CI — order-service")   SVC="order-service";   PORT=8003 ;;
            "CI — api-gateway")     SVC="api-gateway";     PORT=8000 ;;
          esac
      ...
      - name: Helm upgrade — \${{ steps.resolve.outputs.service }}
        id: helm
        run: |
      ...
      - name: Notify Slack
        ...
        with:
          payload: |
            {
              "text": "\${{ job.status == 'success' && '✅' || '❌' }} Staging deploy: *\${{ steps.resolve.outputs.service }}* → \`\${{ steps.resolve.outputs.sha }}\`",
              "blocks": [{
                "type": "section",
                "text": { "type": "mrkdwn",
                  "text": "\${{ job.status == 'success' && '✅ *Staging deploy succeeded*' || '❌ *Staging deploy FAILED*' }}\nService: \`\${{ steps.resolve.outputs.service }}\`\nImage: \`\${{ steps.resolve.outputs.sha }}\`\nEnvironment: *staging*"
                }
              }]
            }
        env:
...
<DiagramBox label="CD Production Flow — Manual Gate + Rolling Deploy">
  <svg viewBox="0 0 860 170" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="219" y="50" textAnchor="middle" fill="#fde047" fontSize="9" fontFamily="monospace" fontWeight="700">⏸ MANUAL APPROVAL</text>
    ...
    <text x="219" y="100" textAnchor="middle" fill="#fde047" fontSize="8" fontFamily="monospace">Required reviewers ≥1</text>
...
<CodeBlock lang="yaml" filename=".github/workflows/cd-production.yml">{`name: CD — Deploy to Production
...
jobs:
        ...
        run: |
          ...
          for SVC in \$SERVICES; do
            ...
            az acr repository show-tags \
              ...
              || { echo "❌ Image \${IMAGE} not found in ACR!"; exit 1; }
  ...
  deploy-production:
    ...
    steps:
      ...
      - name: Audit log — deployment started
        run: |
      ...
      - name: Helm upgrade — production (rolling update)
        id: deploy
        run: |
          TAG="\${{ github.event.inputs.image_tag }}"
          for SVC in \${{ steps.services.outputs.list }}; do
            ...
            echo "🚀 Deploying \${SVC}:\${TAG} to production..."
      ...
      - name: Create GitHub Release
        ...
        with:
          ...
          release_name: "Production Release — \${{ github.event.inputs.image_tag }}"
          body: |
      ...
      - name: Notify Slack — stakeholders
        if: always()
        uses: slackapi/slack-github-action@v1
        with:
          payload: |
            {
              "text": "\${{ job.status == 'success' && '🚀' || '🔥' }} Production deploy: *\${{ github.event.inputs.service }}*",
              "blocks": [
                {
                  "type": "header",
                  "text": { "type": "plain_text", "text": "\${{ job.status == 'success' && '🚀 Production Deploy Succeeded' || '🔥 Production Deploy FAILED' }}" }
                },
                {
                  "type": "section",
                  "fields": [
...
{/* ══════════════════════════════════════════════════
    SECTION 6 — HELM CHART
══════════════════════════════════════════════════ */}
<SectionHeader num="06" title="HELM CHART — KUBERNETES MANIFESTS" sub="One chart, all four services, environment-specific overrides." />

<CodeBlock lang="yaml" filename="helm/shopwave/values.yaml">{`# Default values — overridden per environment
...
{/* ══════════════════════════════════════════════════
    SECTION 7 — TERRAFORM INFRASTRUCTURE AS CODE
══════════════════════════════════════════════════ */}
<SectionHeader num="07" title="TERRAFORM — INFRASTRUCTURE AS CODE" sub="Every Azure resource declared, versioned, and applied through a pipeline." />

<p>All Azure infrastructure is managed by Terraform — no manual portal clicks, no ad-hoc CLI commands. The configuration is split into five focused modules, each owning a single concern. The root <code style={{color:T.accent3}}>main.tf</code> wires them together and additionally provisions GitHub Actions OIDC federation so the CD pipelines can authenticate to Azure without any stored credentials.</p>
...
<DiagramBox label="Terraform Module Dependency Graph">
  <svg viewBox="0 0 860 200" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="425" y="48" textAnchor="middle" fill="#64748b" fontSize="9" fontFamily="monospace">OIDC · namespaces · role assignments</text>
    ...
    <text x="80"  y="153" textAnchor="middle" fill="#64748b" fontSize="8" fontFamily="monospace">VNet · subnets</text>
    <text x="80"  y="168" textAnchor="middle" fill="#64748b" fontSize="8" fontFamily="monospace">NSGs · DNS zone</text>
    ...
    <text x="560" y="153" textAnchor="middle" fill="#64748b" fontSize="8" fontFamily="monospace">cluster · node pools</text>
    <text x="560" y="168" textAnchor="middle" fill="#64748b" fontSize="8" fontFamily="monospace">Log Analytics · Defender</text>
    ...
    <text x="720" y="168" textAnchor="middle" fill="#64748b" fontSize="8" fontFamily="monospace">3 databases → KV</text>
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 12px"}}>infrastructure/terraform/main.tf (root — key excerpts)</h3>
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"0 0 16px"}}>Step 0 — Bootstrap remote state (run once)</h3>
...
# Enable versioning — protects state from accidental overwrites
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 16px"}}>infrastructure/terraform/main.tf — key excerpts</h3>
...
# Kubernetes namespaces — created via k8s provider post-AKS provisioning
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 16px"}}>modules/aks/main.tf — production-hardened cluster</h3>
...
<CodeBlock lang="hcl" filename="infrastructure/terraform/modules/aks/main.tf">{`resource "azurerm_kubernetes_cluster" "main" {
  ...
  default_node_pool {               # system pool — kube-system only
    name                         = "system"
...
# App node pool — user workloads land here (no taint)
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 16px"}}>modules/postgres/main.tf — secrets auto-stored in Key Vault</h3>

<p>The postgres module generates a random password, provisions the Flexible Server with per-service databases, and writes the full connection strings directly into Key Vault — so they're available to the External Secrets Operator without any manual copy-paste step.</p>
...
# Write connection strings into Key Vault — one per database
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 16px"}}>infrastructure/terraform/variables.tf — fully validated</h3>
...
<h3 style={{fontFamily:"monospace",color:T.accent,margin:"32px 0 16px"}}>infrastructure/terraform/outputs.tf — reads straight into GitHub Secrets</h3>
...
<CodeBlock lang="bash" filename="terminal">{`# 1 — Bootstrap remote state (once only)
...
{/* ══════════════════════════════════════════════════
    SECTION 9 — SECRETS REFERENCE
══════════════════════════════════════════════════ */}
<SectionHeader num="09" title="GITHUB SECRETS REFERENCE" sub="Every secret the pipelines need — where it comes from, what it does." />
...
<DiagramBox label="Required GitHub Secrets">
  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.85rem", fontFamily: "monospace" }}>
    ...
    <tbody>
      {[
        ["AZURE_CLIENT_ID",       "terraform output github_actions_client_id",  "All pipelines",             "OIDC federated identity — no password"],
        ...
        ["SLACK_WEBHOOK_URL",     "Slack app → Incoming Webhooks",              "All pipelines (notify)",    "Incoming webhook URL"],
...
{/* ══════════════════════════════════════════════════
    SECTION 10 — END-TO-END DEVELOPER WORKFLOW
══════════════════════════════════════════════════ */}
<SectionHeader num="10" title="END-TO-END DEVELOPER WORKFLOW" sub="From git commit to live production — every step automated." />

<DiagramBox label="Complete Developer → Production Journey">
  <svg viewBox="0 0 860 590" width="100%" xmlns="http://www.w3.org/2000/svg">
    ...
    {[
      { y: 40,  icon: "1", color: "#a855f7", title: "Developer opens PR",              detail: "git checkout -b feat/add-wishlist → git push → PR opened",                          time: "T+0" },
      { y: 110, icon: "2", color: "#3b82f6", title: "CI triggers (path-filtered)",     detail: "Only product-service CI runs · lint → test → SAST · ~6 min",                      time: "T+6m" },
      { y: 180, icon: "3", color: "#22c55e", title: "Tests pass · PR reviewable",      detail: "GitHub status checks green · Coverage badge updates · Codecov report",             time: "T+7m" },
      { y: 250, icon: "4", color: "#eab308", title: "Code review + merge to main",     detail: "Reviewer approves · Squash merge · CI re-runs on main branch",                     time: "T+Xh" },
      { y: 320, icon: "5", color: "#f97316", title: "CI builds + pushes to ACR",       detail: "Multi-stage Docker build · Trivy scan (block on CRITICAL) · Push sha-XXXXXXXX",    time: "T+Xh+8m" },
      { y: 390, icon: "6", color: "#3b82f6", title: "CD staging auto-deploys",         detail: "Helm upgrade --atomic · Smoke tests · Slack alert · staging tag created",           time: "T+Xh+13m" },
      { y: 460, icon: "7", color: "#22c55e", title: "Manual gate → production",        detail: "Engineer triggers cd-production.yml · Reviewer approves · Rolling deploy → live",   time: "T+Xh+30m" },
      { y: 530, icon: "★", color: "#ec4899", title: "Infra change? terraform.yml",     detail: "PR touches infrastructure/terraform/ → plan comment on PR → apply on dispatch",     time: "as needed" },
...
{/* ══════════════════════════════════════════════════
    SECTION 11 — NETWORK POLICIES & SECURITY
══════════════════════════════════════════════════ */}
...
# Allow ingress from nginx ingress controller → api-gateway only
---
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
...
# Allow api-gateway → each backend service
---
apiVersion: networking.k8s.io/v1
kind: NetworkPolicy
metadata:
...
{/* ══════════════════════════════════════════════════
    SECTION 12 — PIPELINE SECURITY SUMMARY
══════════════════════════════════════════════════ */}
...
<DiagramBox label="Security Controls — Layer by Layer">
  <svg viewBox="0 0 860 430" width="100%" xmlns="http://www.w3.org/2000/svg">
    {[
      ...
      { y: 110, label: "IMAGE",       color: "#3b82f6", items: ["Multi-stage build — distroless final image (~15MB)", "Trivy scans CRITICAL/HIGH CVEs — blocks pipeline", "Cosign image signing — ACR verifies before deploy"] },
      { y: 200, label: "INFRA",       color: "#ec4899", items: ["All Azure resources declared in Terraform — no manual changes", "terraform plan diff commented on every infra PR", "Remote state in Azure Blob with versioning enabled"] },
      { y: 290, label: "RUNTIME",     color: "#22c55e", items: ["runAsNonRoot: true · readOnlyRootFilesystem: true", "capabilities: drop [ALL] — zero Linux capabilities", "Azure Network Policy — default deny, explicit allow-list"] },
      { y: 380, label: "CREDENTIALS", color: "#eab308", items: ["OIDC Workload Identity — zero static credentials in pipelines", "Azure Key Vault — connection strings auto-seeded by Terraform", "Secret rotation via Key Vault versioning + ESO 1h refresh"] },
      ...
      <g key={y}>
        ...
        {items.map((item, i) => (
          <text key={i} x="130" y={y + 22 + i * 20} fill="#94a3b8" fontSize="10" fontFamily="monospace">
            <tspan fill={color} fontSize="10">▸ </tspan>{item}
          </text>
        ))}
      </g>
    ))}
...
<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "14px", margin: "28px 0" }}>
  {[
    { icon: "🐍", title: "4 Python FastAPI Services",        desc: "auth, product, order, api-gateway — each with async SQLAlchemy, Alembic migrations, structured logging, Prometheus metrics" },
    { icon: "⚙️",  title: "7 GitHub Actions Pipelines",      desc: "4 per-service CI pipelines + staging CD + production CD + terraform.yml — path-filtered, OIDC-authenticated, Slack-notified" },
    { icon: "🏗️", title: "Terraform Infrastructure as Code", desc: "5 modules — networking, aks, acr, postgres, keyvault — all resources versioned, plan-reviewed on PR, applied through a gated pipeline" },
    { icon: "🔒", title: "End-to-End Security",              desc: "Semgrep SAST · Trivy image scan · Cosign signing · OIDC no-credential auth · Key Vault secrets auto-seeded by Terraform · zero-trust network policies" },
    { icon: "☸️",  title: "Helm Chart + K8s Manifests",      desc: "Single parameterised chart · rolling update strategy · HPA · PDB · External Secrets · RBAC · Network Policies" },
    { icon: "🚀", title: "Zero-Downtime Deployments",        desc: "maxUnavailable: 0 rolling updates · --atomic Helm with auto-rollback · smoke tests gate every staging deploy" },
    { icon: "☁️",  title: "Full Azure Stack",                desc: "AKS + ACR + PostgreSQL Flexible (zone-redundant in prod) + Key Vault + private VNet + Azure Monitor + Defender for Containers — all Terraform-managed" },
  ].map(({ icon, title, desc }) => (
...
<div style={{ borderTop: `1px solid ${T.border}`, marginTop: "60px", paddingTop: "32px", textAlign: "center" }}>
  <p style={{ fontFamily: "monospace", fontSize: "12px", color: T.muted }}>ShopWave · Python FastAPI · Azure AKS · Terraform IaC · GitHub Actions · Production-Grade CI/CD · February 2026</p>
</div>
