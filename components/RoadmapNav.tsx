    ...
    <div ref={topRef} style={{ scrollMarginTop: '80px' }}>
      ...
      >
        <button
          ...
          style={ts(active === 'overview', '#f59e0b', true)}
        >
          Overview
        </button>
      ...
      {active === 'overview' && (
        <div style={{ marginTop: '32px' }}>
          ...
          >
            ...
            >
              Six modules. Each builds on the last. Technical skill gets you in the room — the full
              stack of skills here determines what you do once you&apos;re there.
            </p>
          </div>
          ...
          >
            {MODULES.map((m) => (
              <div key={m.id} style={{ flex: 1, background: m.color, opacity: 0.7 }} />
            ))}
          </div>
          ...
          <div style={{ borderTop: '1px solid #27272a', paddingTop: '28px' }}>
            ...
            >
              {[
                {
                  n: '01',
                  t: 'Think before typing',
                  d: 'Five minutes of design saves fifty of debugging.',
                  c: '#22d3ee',
                },
                {
                  n: '02',
                  t: 'Tradeoffs everywhere',
                  d: 'Every decision has a cost. Name it explicitly.',
                  c: '#f59e0b',
                },
                {
                  n: '03',
                  t: 'Make it work first',
                  d: 'Correct before fast. Simple before clever.',
                  c: '#34d399',
                },
                {
                  n: '04',
                  t: 'Observe everything',
                  d: 'You cannot improve what you cannot measure.',
                  c: '#a78bfa',
                },
                {
                  n: '05',
                  t: 'Communication is leverage',
                  d: 'A doc touches 50 engineers. Code touches 5.',
                  c: '#60a5fa',
                },
              ].map((p) => (
                <div
      ...
      {active !== 'overview' && mod && (
        ...
        >
          <div>
            ...
            <span style={badge(mod.color)}>
              Module {mod.num} &nbsp; {mod.phase}
            </span>
            <h2
              style={{
                margin: '10px 0 0',
          ...
          <div style={{ display: 'flex', gap: '8px', flexShrink: 0, marginTop: '4px' }}>
            {idx > 0 && (
              <button
                onClick={() => navigate(MODULES[idx - 1].id)}
                style={nb('#a1a1aa', '#3f3f46')}
              >
                ← Prev
              </button>
            )}
            {idx < MODULES.length - 1 && (
              <button
                onClick={() => navigate(MODULES[idx + 1].id)}
                style={nb(mod.color, mod.color, true)}
              >
                Next →
              </button>
            )}
...
function ModuleCard({ m, onClick }: { m: (typeof MODULES)[0]; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <p style={{ margin: '0 0 10px', fontWeight: 700, fontSize: '14px', color: '#f4f4f5' }}>
        {m.label}
      </p>
      <p style={{ margin: 0, fontSize: '12px', color: m.color, fontFamily: 'monospace' }}>
        Open module →
      </p>
    </button>
  )
