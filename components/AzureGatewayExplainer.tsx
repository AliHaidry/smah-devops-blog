...
const COMPARISON_ROWS: [string, string, string][] = [
  ...
  ['Subnet Type',               'Public Subnet (internet route in table)',      'Private Subnet (NAT GW in route table)'],
  ['Primary Use Case',          'Web servers, Bastion Hosts, Load Balancers',   'App servers, DB patching, package downloads'],
  ['Security Posture',          'NSG required to restrict inbound access',      'Inherently private — no inbound attack surface'],
  ['SNAT Behaviour',            'Default SNAT (unpredictable public IP)',        'Dedicated, predictable static Public IP'],
  ['Scale',                     '1 Public IP per NIC (multiple attachable)',     'Up to 16 Public IPs · 64 K SNAT ports/IP'],
  ...
  ['SNAT Port Exhaustion Risk', 'High (default SNAT has limited ports)',          'Low (64 K ports/IP, add IPs to scale)'],
]
...
function SectionLabel({ children }: { children: ReactNode }) {
  return (
    <p className="mb-4 font-mono text-[10px] uppercase tracking-[3px] text-sky-400/70">
      {children}
    </p>
  )
}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-[1fr_56px_1fr] overflow-hidden rounded-2xl border border-slate-700/60">
        ...
        <div className="agx-c1 relative bg-slate-900 p-7">
          ...
          <p className="mb-4 font-mono text-[11px] text-slate-500">// Inbound + Outbound public traffic</p>
      ...
      {/* Golden rule */}
      <div className="agx-c3 agx-shimmer flex gap-4 rounded-xl border border-sky-800/40 p-5"
        ...
        <div>
          ...
          <p className="text-[13px] leading-relaxed text-slate-400">
            <strong className="text-slate-200">Internet Gateway = someone on the internet can reach YOU.</strong>{' '}
            <strong className="text-slate-200">NAT Gateway = YOU can reach the internet, but nobody can reach you back.</strong>{' '}
            In production, your web tier uses Public IPs, your app/DB tier uses NAT Gateway (or no
            internet at all). This is the <strong className="text-slate-200">defense-in-depth</strong> principle —
            minimise your attack surface by keeping backend resources private.
          </p>
        </div>
      </div>
    </div>
      ...
      <div className="agx-c1 overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900">
        ...
        <div className="p-6">
          <svg viewBox="0 0 360 395" className="w-full" aria-label="Internet Gateway flow diagram">
            {/* VNet */}
            <rect x="10" y="96" width="340" height="289" rx="12"
                  fill="rgba(15,23,42,0.85)" stroke="#1e3a5f" strokeWidth="1.5"/>
            <text x="22" y="114" fill="#334155" fontSize="9" fontFamily="monospace">Azure VNet</text>
            {/* Public Subnet */}
            <rect x="28" y="124" width="304" height="251" rx="8"
                  fill="rgba(124,58,237,0.06)" stroke="rgba(167,139,250,0.3)"
                  strokeWidth="1" strokeDasharray="5 3"/>
            <text x="40" y="141" fill="rgba(167,139,250,0.55)" fontSize="8" fontFamily="monospace">
              Public Subnet  10.0.1.0/24
            </text>
            {/* Internet cloud */}
            <circle cx="180" cy="40" r="30" fill="rgba(124,58,237,0.14)"
                    stroke="rgba(167,139,250,0.55)" strokeWidth="1.5"/>
            <text x="180" y="36" fontSize="13" textAnchor="middle">🌐</text>
            <text x="180" y="52" fill="#a78bfa" fontSize="8" textAnchor="middle" fontFamily="monospace">Internet</text>
            {/* Public IP pill */}
            <rect x="118" y="80" width="124" height="28" rx="7"
                  fill="rgba(167,139,250,0.15)" stroke="rgba(167,139,250,0.45)" strokeWidth="1"/>
            <text x="180" y="93" fill="#c4b5fd" fontSize="7.5" textAnchor="middle" fontFamily="monospace">Public IP</text>
            <text x="180" y="104" fill="#a78bfa" fontSize="7.5" textAnchor="middle" fontFamily="monospace">20.50.100.10</text>

            {/* Animated flowing lines — bidirectional */}
            <line x1="180" y1="70" x2="180" y2="79" stroke="#a78bfa" strokeWidth="2.2" className="agx-flow-fwd"/>
            <line x1="175" y1="79" x2="175" y2="70" stroke="#c4b5fd" strokeWidth="1.3" className="agx-flow-bwd" opacity="0.55"/>
            <line x1="180" y1="108" x2="180" y2="164" stroke="#a78bfa" strokeWidth="2" className="agx-flow-fwd"/>
            <line x1="175" y1="164" x2="175" y2="108" stroke="#c4b5fd" strokeWidth="1.3" className="agx-flow-bwd" opacity="0.5"/>
            ...
            <circle cx="180" cy="74" r="3.8" fill="#a78bfa" opacity="0.85" className="agx-pkt-down"/>

            {/* VM box */}
            <rect x="85" y="165" width="190" height="68" rx="10"
                  fill="rgba(15,23,42,0.95)" stroke="rgba(167,139,250,0.55)" strokeWidth="1.5"/>
            <text x="180" y="190" fontSize="16" textAnchor="middle">🖥️</text>
            <text x="180" y="210" fill="#e2e8f0" fontSize="10" textAnchor="middle"
                  fontFamily="sans-serif" fontWeight="600">VM / Web Server</text>
            <text x="180" y="223" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
              10.0.1.4  ·  NIC with Public IP
            </text>

            {/* NSG */}
            <rect x="48" y="254" width="264" height="36" rx="8"
                  fill="rgba(251,146,60,0.07)" stroke="rgba(251,146,60,0.3)" strokeWidth="1"/>
            <text x="180" y="269" fill="#fb923c" fontSize="8" textAnchor="middle" fontFamily="monospace">🛡  NSG</text>
            <text x="180" y="281" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
              Inbound: Allow 80, 443  ·  Outbound: Allow All
            </text>
            {/* Route Table */}
            <rect x="48" y="300" width="264" height="30" rx="8"
                  fill="rgba(96,165,250,0.07)" stroke="rgba(96,165,250,0.27)" strokeWidth="1"/>
            <text x="180" y="313" fill="#60a5fa" fontSize="8" textAnchor="middle" fontFamily="monospace">📋  Route Table</text>
            <text x="180" y="324" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
          ...
          <div className="mt-3 rounded-lg border border-violet-900/40 bg-violet-950/20 p-3">
            ...
            <p className="text-[11px] leading-relaxed text-slate-500">
              ...
              <span className="text-violet-300">NSG</span> controls what inbound traffic is permitted.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="agx-c2 overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900">
        ...
        <div className="p-6">
          <svg viewBox="0 0 360 395" className="w-full" aria-label="NAT Gateway flow diagram">
            ...
            {/* Private Subnet */}
            <rect x="28" y="124" width="304" height="251" rx="8"
                  fill="rgba(0,180,216,0.04)" stroke="rgba(34,211,238,0.22)"
                  strokeWidth="1" strokeDasharray="5 3"/>
            <text x="40" y="141" fill="rgba(34,211,238,0.45)" fontSize="8" fontFamily="monospace">
              Private Subnet  10.0.2.0/24
            </text>
            {/* Internet cloud */}
            <circle cx="180" cy="40" r="30" fill="rgba(0,180,216,0.09)"
                    stroke="rgba(34,211,238,0.4)" strokeWidth="1.5"/>
            <text x="180" y="36" fontSize="13" textAnchor="middle">🌐</text>
            <text x="180" y="52" fill="#22d3ee" fontSize="8" textAnchor="middle" fontFamily="monospace">Internet</text>

            {/* NAT Gateway pill */}
            <rect x="106" y="78" width="148" height="32" rx="8"
                  fill="rgba(0,214,143,0.1)" stroke="rgba(0,214,143,0.52)" strokeWidth="1.8"/>
            <text x="180" y="93" fill="#00d68f" fontSize="9" textAnchor="middle"
                  fontFamily="sans-serif" fontWeight="700">🔄  NAT Gateway</text>
            <text x="180" y="105" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
              Public IP: 20.50.200.99
            </text>

            {/* Outbound-only animated line (upward) */}
            <line x1="180" y1="78" x2="180" y2="70" stroke="#00d68f" strokeWidth="2.5" className="agx-flow-bwd"/>
            <polygon points="180,70  175,77  185,77" fill="#00d68f"/>

            {/* VM → NAT outbound */}
            <line x1="180" y1="112" x2="180" y2="164" stroke="#00d68f" strokeWidth="2" className="agx-flow-bwd"/>
            ...
            <g className="agx-blocked">
              ...
              <text x="132" y="47" fill="#f87171" fontSize="7" fontFamily="monospace">BLOCKED</text>
            </g>

            {/* VM box */}
            <rect x="85" y="165" width="190" height="68" rx="10"
                  fill="rgba(15,23,42,0.95)" stroke="rgba(34,211,238,0.4)" strokeWidth="1.5"/>
            <text x="180" y="190" fontSize="16" textAnchor="middle">⚙️</text>
            <text x="180" y="210" fill="#e2e8f0" fontSize="10" textAnchor="middle"
                  fontFamily="sans-serif" fontWeight="600">VM / App Server</text>
            <text x="180" y="223" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
            ...
            {/* NSG */}
            <rect x="48" y="254" width="264" height="36" rx="8"
                  fill="rgba(251,146,60,0.06)" stroke="rgba(251,146,60,0.24)" strokeWidth="1"/>
            <text x="180" y="269" fill="#fb923c" fontSize="8" textAnchor="middle" fontFamily="monospace">🛡  NSG</text>
            <text x="180" y="281" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
            ...
            {/* Route Table */}
            <rect x="48" y="300" width="264" height="30" rx="8"
                  fill="rgba(0,214,143,0.06)" stroke="rgba(0,214,143,0.24)" strokeWidth="1"/>
            <text x="180" y="313" fill="#00d68f" fontSize="8" textAnchor="middle" fontFamily="monospace">📋  Route Table</text>
            <text x="180" y="324" fill="#475569" fontSize="7" textAnchor="middle" fontFamily="monospace">
...
function Production() {
  ...
  return (
    <div className="agx-c1 overflow-hidden rounded-2xl border border-slate-700/60 bg-slate-900 p-7">
      ...
      <svg viewBox="0 0 920 302" className="w-full" aria-label="Production 3-tier Azure architecture">
        {/* Internet */}
        <circle cx="56" cy="151" r="44" fill="rgba(124,58,237,0.08)"
                stroke="rgba(167,139,250,0.4)" strokeWidth="1.5"/>
        <text x="56" y="146" fontSize="16" textAnchor="middle">🌐</text>
        <text x="56" y="163" fill="#a78bfa" fontSize="8" textAnchor="middle" fontFamily="monospace">Internet</text>

        {/* VNet */}
        <rect x="116" y="18" width="790" height="266" rx="14"
              fill="rgba(15,23,42,0.6)" stroke="#1e3a5f" strokeWidth="1.5"/>
        <text x="132" y="36" fill="#334155" fontSize="9" fontFamily="monospace">Azure VNet  10.0.0.0/16</text>

        {/* Public Subnet */}
        <rect x="132" y="46" width="206" height="218" rx="10"
              fill="rgba(124,58,237,0.05)" stroke="rgba(167,139,250,0.28)"
              strokeWidth="1" strokeDasharray="4 3"/>
        <text x="235" y="62" fill="rgba(167,139,250,0.45)" fontSize="8"
              textAnchor="middle" fontFamily="monospace">Public Subnet</text>
        <rect x="158" y="72" width="154" height="46" rx="8"
              fill="rgba(15,23,42,0.95)" stroke="rgba(167,139,250,0.5)" strokeWidth="1.5"/>
        <text x="235" y="92" fontSize="12" textAnchor="middle">⚖️</text>
        <text x="235" y="109" fill="#c4b5fd" fontSize="9" textAnchor="middle"
              fontFamily="sans-serif" fontWeight="600">Load Balancer</text>
        <rect x="150" y="136" width="64" height="46" rx="6"
              fill="rgba(15,23,42,0.85)" stroke="rgba(167,139,250,0.3)" strokeWidth="1"/>
        <text x="182" y="156" fontSize="10" textAnchor="middle">🖥️</text>
        <text x="182" y="172" fill="#94a3b8" fontSize="7" textAnchor="middle" fontFamily="monospace">Web VM</text>
        <rect x="258" y="136" width="64" height="46" rx="6"
              fill="rgba(15,23,42,0.85)" stroke="rgba(167,139,250,0.3)" strokeWidth="1"/>
        <text x="290" y="156" fontSize="10" textAnchor="middle">🖥️</text>
        <text x="290" y="172" fill="#94a3b8" fontSize="7" textAnchor="middle" fontFamily="monospace">Web VM</text>

        {/* Private Subnet */}
        <rect x="370" y="46" width="206" height="218" rx="10"
              fill="rgba(0,180,216,0.04)" stroke="rgba(34,211,238,0.22)"
              strokeWidth="1" strokeDasharray="4 3"/>
        <text x="473" y="62" fill="rgba(34,211,238,0.45)" fontSize="8"
              textAnchor="middle" fontFamily="monospace">Private Subnet</text>
        <rect x="384" y="72" width="64" height="46" rx="6"
              fill="rgba(15,23,42,0.85)" stroke="rgba(34,211,238,0.32)" strokeWidth="1"/>
        <text x="416" y="92" fontSize="10" textAnchor="middle">⚙️</text>
        <text x="416" y="108" fill="#94a3b8" fontSize="7" textAnchor="middle" fontFamily="monospace">App VM</text>
        <rect x="494" y="72" width="64" height="46" rx="6"
              fill="rgba(15,23,42,0.85)" stroke="rgba(34,211,238,0.32)" strokeWidth="1"/>
        <text x="526" y="92" fontSize="10" textAnchor="middle">⚙️</text>
        <text x="526" y="108" fill="#94a3b8" fontSize="7" textAnchor="middle" fontFamily="monospace">App VM</text>
        <rect x="386" y="148" width="156" height="44" rx="8"
              fill="rgba(0,214,143,0.08)" stroke="rgba(0,214,143,0.44)" strokeWidth="1.5"/>
        <text x="464" y="167" fontSize="11" textAnchor="middle">🔄</text>
        <text x="464" y="183" fill="#00d68f" fontSize="8" textAnchor="middle" fontFamily="monospace">NAT Gateway</text>

        {/* DB Subnet */}
        <rect x="610" y="46" width="280" height="218" rx="10"
              fill="rgba(255,209,102,0.03)" stroke="rgba(255,209,102,0.18)"
              strokeWidth="1" strokeDasharray="4 3"/>
        <text x="750" y="62" fill="rgba(255,209,102,0.38)" fontSize="8"
              textAnchor="middle" fontFamily="monospace">DB Subnet  (fully private)</text>
        <rect x="628" y="78" width="172" height="56" rx="8"
              fill="rgba(15,23,42,0.95)" stroke="rgba(255,209,102,0.34)" strokeWidth="1.5"/>
        <text x="714" y="102" fontSize="14" textAnchor="middle">🗄️</text>
        <text x="714" y="122" fill="#fbbf24" fontSize="9" textAnchor="middle"
              fontFamily="sans-serif" fontWeight="600">PostgreSQL Flexible</text>
        <rect x="628" y="150" width="172" height="54" rx="8"
              fill="rgba(15,23,42,0.95)" stroke="rgba(251,146,60,0.3)" strokeWidth="1.5"/>
        <text x="714" y="173" fontSize="12" textAnchor="middle">🔑</text>
        <text x="714" y="193" fill="#fb923c" fontSize="9" textAnchor="middle"
              fontFamily="sans-serif" fontWeight="600">Key Vault</text>

        {/* ── Animated arrows ── */}

        {/* Internet ↔ LB  (bidirectional) */}
        <line x1="100" y1="140" x2="156" y2="106"
              stroke="#a78bfa" strokeWidth="2" className="agx-flow-fwd"/>
        <line x1="103" y1="145" x2="159" y2="111"
              stroke="#c4b5fd" strokeWidth="1.2" className="agx-flow-bwd" opacity="0.5"/>
        <polygon points="156,106 147,107 151,115" fill="#a78bfa"/>
        <polygon points="100,140 108,133 104,142" fill="#c4b5fd" opacity="0.65"/>

        {/* LB → Web VMs */}
        <line x1="212" y1="118" x2="190" y2="134"
              stroke="#a78bfa" strokeWidth="1.5" className="agx-flow-fwd"/>
        <polygon points="190,134 185,126 194,126" fill="#a78bfa"/>
        <line x1="258" y1="118" x2="278" y2="134"
              stroke="#a78bfa" strokeWidth="1.5" className="agx-flow-fwd"/>
        <polygon points="278,134 273,126 282,126" fill="#a78bfa"/>

        {/* Web → App  internal */}
        <line x1="338" y1="97" x2="382" y2="97"
              stroke="#60a5fa" strokeWidth="2" className="agx-flow-fwd"/>
        <polygon points="382,97 374,93 374,101" fill="#60a5fa"/>
        <text x="360" y="91" fill="#60a5fa" fontSize="7" textAnchor="middle" fontFamily="monospace">internal</text>

        {/* App → NAT */}
        <line x1="437" y1="118" x2="437" y2="146"
              stroke="#00d68f" strokeWidth="1.6" className="agx-flow-bwd"/>
        <polygon points="437,118 432,126 442,126" fill="#00d68f"/>

        {/* NAT → Internet  outbound only */}
        <line x1="384" y1="166" x2="120" y2="158"
              stroke="#00d68f" strokeWidth="1.8" className="agx-flow-bwd"/>
        <polygon points="120,158 129,154 128,162" fill="#00d68f"/>
        <text x="254" y="178" fill="#00d68f" fontSize="7" textAnchor="middle" fontFamily="monospace">outbound only</text>

        {/* App → DB  private link */}
        <line x1="576" y1="97" x2="626" y2="107"
              stroke="#fbbf24" strokeWidth="1.6" className="agx-flow-fwd"/>
        <polygon points="626,107 618,101 618,110" fill="#fbbf24"/>
        <text x="601" y="92" fill="#fbbf24" fontSize="7" textAnchor="middle" fontFamily="monospace">private link</text>

        {/* Travelling packet dots */}
        <circle cx="128" cy="149" r="3.5" fill="#a78bfa" opacity="0.8"
                style={{ animation: 'agx-dash-fwd 2s linear infinite' }}/>
        <circle cx="437" cy="138" r="3.2" fill="#00d68f" opacity="0.85"
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
        {tiers.map((t, i) => (
          <div key={t.label}
               className={`agx-c${i + 1} rounded-lg border p-3 ${t.border} ${t.bg}`}>
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-slate-800/70">
                {headers.map((h, i) => (
                  <th key={h}
                      className={`border-b border-slate-700/60 px-5 py-3 text-left font-mono text-[10px] uppercase tracking-wide ${
            ...
            <tbody>
              {COMPARISON_ROWS.map(([attr, internet, nat]) => (
                <tr key={attr} className="agx-row border-b border-slate-800/60 last:border-0">
                  <td className="whitespace-nowrap px-5 py-3 font-mono text-[11px] text-slate-500">{attr}</td>
                  <td className="px-5 py-3 text-[12px] leading-relaxed text-violet-200/80">{internet}</td>
    ...
    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
      {columns.map((col, ci) => (
        <div key={col.title}
          ...
          <div className={`absolute inset-x-0 bottom-0 h-[2px] bg-gradient-to-r ${col.bottomGradient}`} />
          ...
          <div className="space-y-3">
            {col.scenarios.map((s) => (
              <div key={s.title}
                ...
                <p className={`mb-1 font-mono text-[11px] font-bold ${col.accentText}`}>{s.title}</p>
    ...
    <>
      ...
      <div className="relative my-8 overflow-hidden rounded-3xl border border-slate-700/50 bg-[#0a0e1a] p-6 shadow-2xl sm:p-8">
        ...
        <div className={`relative mb-9 text-center ${mounted ? 'agx-hdr' : 'opacity-0'}`}>
          <span className="mb-3 inline-block rounded-full border border-sky-800/60 bg-sky-900/30 px-3 py-1 font-mono text-[10px] uppercase tracking-[2px] text-sky-400">
            Azure Networking
          </span>
          <h2 className="mb-2 bg-gradient-to-br from-white to-sky-300 bg-clip-text font-sans text-3xl font-extrabold leading-tight text-transparent sm:text-4xl">
        ...
        {/* Tab bar */}
        <div className={`relative mb-7 flex gap-1 overflow-x-auto rounded-xl border border-slate-700/60 bg-slate-800/50 p-1 ${mounted ? 'agx-tabs' : 'opacity-0'}`}>
          {TABS.map((t) => (
            <button
              key={t.id}
              onClick={() => setActiveTab(t.id)}
              className={`flex-1 whitespace-nowrap rounded-lg px-3 py-2 font-mono text-[11px] transition-all duration-200 ${
                activeTab === t.id
        ...
        {/* Content */}
        <div className={`relative ${mounted ? 'agx-body' : 'opacity-0'}`}>
          {content[activeTab]}
        </div>
      </div>
    </>
